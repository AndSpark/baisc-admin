...
const style = computed(() =>
  props.useMinWidthLayout ? `min-width:${props.minWidth}px;` : ''
)
...
const siderZIndex = computed(() =>
  props.isMobile || isVertical.value ? 1003 : 1000
)
...
const commonPaddingLeft = computed(() =>
  props.isMobile ? 0 : currentSiderWidth.value
)

// 各子组件的属性
const headerPaddingLeft = computed(() =>
  isVertical.value ? commonPaddingLeft.value : 0
)
const siderPaddingTop = computed(() =>
  !props.isMobile && !isVertical.value && props.headerVisible
    ? props.headerHeight
    : 0
)
